...
class ServiceRegistration(Base):
    ...
    repair_details: Mapped[str] = mapped_column(Text, nullable=False)
...
class ServicePhoto(Base):
    ...
    service_registration_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("service_registrations.id", ondelete="CASCADE"),
        nullable=False,
    )
