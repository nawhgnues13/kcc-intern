...
class GroomingRegistration(Base):
    __tablename__ = "grooming_registrations"
    ...
    pet_type: Mapped[Optional[str]] = mapped_column(String(20), nullable=True)
    breed: Mapped[Optional[str]] = mapped_column(String(120), nullable=True)
    grooming_details: Mapped[str] = mapped_column(Text, nullable=False)
    price: Mapped[Optional[Decimal]] = mapped_column(Numeric(12, 2), nullable=True)
    grooming_date: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
...
class GroomingPhoto(Base):
    __tablename__ = "grooming_photos"
    ...
    grooming_registration_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("grooming_registrations.id", ondelete="CASCADE"),
        nullable=False,
    )
