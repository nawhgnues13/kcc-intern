...
import { Plus, Search, Scissors, Database } from "lucide-react";
          ...
          <div className="flex flex-col h-[85vh] bg-white rounded-3xl overflow-hidden relative">
            ...
            <div className="shrink-0 px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-white z-20">
              <h2 className="text-xl font-bold text-slate-800">
                {isViewMode ? '미용 내역 상세' : '내용 수정 및 재생성'}
              </h2>
            </div>
              ...
              <div className="space-y-6">
                  ...
                  <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-x-8 gap-y-4">
                      {isEditMode && !snapshot ? (
                        <>
                          ...
                          <div className="col-span-1">
                            <label className="text-xs font-bold text-slate-600 block mb-1">연락처</label>
                          ...
                          <div className="col-span-1">
                            <label className="text-xs font-bold text-slate-600 block mb-1">반려동물 이름</label>
                          ...
                          <div className="col-span-1">
                            <label className="text-xs font-bold text-slate-600 block mb-1">반려동물 종류</label>
                          ...
                          <div className="col-span-1">
                            <label className="text-xs font-bold text-slate-600 block mb-1">품종</label>
                          ...
                          <div className="col-span-1">
                            <label className="text-xs font-bold text-slate-600 block mb-1">미용사</label>
                          ...
                          <div className="col-span-1">
                            <label className="text-xs font-bold text-slate-600 block mb-1">미용 일시</label>
                          ...
                          <div className="col-span-1">
                            <label className="text-xs font-bold text-slate-600 block mb-1">결제 금액</label>
                            <input required type="number" value={formData.price} onChange={e => setFormData({...formData, price: Number(e.target.value)})} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm bg-white" />
                          </div>
                          <div className="col-span-1">
                            <label className="text-xs font-bold text-slate-600 block mb-1">지점</label>
                        ...
                        <>
                          ...
                          <div><p className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1">반려동물 / 품종</p><p className="text-sm font-semibold text-slate-700">{formData.pet_name || '-'} ({formData.breed || '-'})</p></div>
                          <div><p className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1">미용 일시</p><p className="text-sm font-semibold text-slate-700">{formData.grooming_date?.replace('T', ' ') || '-'}</p></div>
                          <div><p className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1">결제 금액</p><p className="text-sm font-semibold text-slate-700">{Number(formData.price || 0).toLocaleString()}원</p></div>
                          <div><p className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1">지점</p><p className="text-sm font-semibold text-slate-700">{formData.branch_name || '-'}</p></div>
                        </>
                      )}
                    ...
                    <div className="space-y-4">
                        <div>
                            <label className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-2">미용 상세 내역</label>
                            {isEditMode ? (
                                <textarea required value={formData.grooming_details} onChange={e => setFormData({...formData, grooming_details: e.target.value})} rows={3} className="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm bg-white resize-none" />
                            ) : (
