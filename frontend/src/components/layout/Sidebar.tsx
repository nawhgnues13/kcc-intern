...
export function Sidebar() {
  ...
  const navItems = [
    ...
    { label: "AI 콘텐츠 생성", path: "/dashboard", icon: <PenSquare className="w-5 h-5" /> },
    { label: "전체 기사", path: "/articles", icon: <BookOpen className="w-5 h-5" /> },
    { label: "생성 기록", path: "/history", icon: <Clock className="w-5 h-5" /> },
  ...
  const crmItems = [
    {
      ...
      visible: user?.ui_permissions?.can_manage_employees,
    },
    {
      label: "차량 판매 관리",
      path: "/crm/sales",
      icon: <Car className="w-5 h-5" />,
      visible: user?.ui_permissions?.can_manage_sales,
    },
    {
      label: "매출 트렌드 분석",
      path: "/crm/trends",
    ...
    },
    {
      ...
      visible: user?.ui_permissions?.can_manage_service,
    },
    {
      label: "애견 미용 관리",
      path: "/crm/grooming",
      icon: <Scissors className="w-5 h-5" />,
      visible: user?.ui_permissions?.can_manage_grooming,
    },
  ...
  return (
    <aside className="w-64 bg-white border-r border-slate-200 h-full flex flex-col flex-shrink-0 z-20">
      ...
      <div className="flex-1 overflow-y-auto py-6 px-4 space-y-6">
          ...
          <div className="space-y-1">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <div className={isActive ? "text-[#3721ED]" : "text-slate-400"}>{item.icon}</div>
                  {item.label}
                </Link>
              );
            })}
        ...
        {crmItems.length > 0 && (
          <div>
            <div className="text-xs font-semibold text-slate-400 mb-3 px-2 uppercase tracking-wider">
              상시 DB (운영)
            </div>
            <div className="space-y-1">
              {crmItems.map((item) => {
